...
func (questionnaire *GitContextQuestionnaire) AskAboutGitContext(installOptions *entity.InstallCmdOptions) error {
	...
	if len(list) == 1 {
		...
		err, installOptions.Git.Integration = prompt.NewPrompt().Select(list, "Select Git integration (Please create a dedicated context for the agent to avoid hitting the Github rate limits or use github app integration)")
	}
