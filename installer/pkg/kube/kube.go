...
func (k *kube) GetAutogeneratedPassword() (string, error) {
	...
	podsList, err := k.clientSet.CoreV1().Pods(k.namespace).List(context.Background(), opts)
	if err != nil {
		return "", err
	}
...
func (k *kube) GetService(labelSelector string) (*core.Service, error) {
	...
	svcs, err := k.clientSet.CoreV1().Services(k.namespace).List(context.Background(), opts)
...
func (k *kube) GetArgoServerSvc(namespace string) (core.Service, error) {
	...
	svcs, err := k.clientSet.CoreV1().Services(namespace).List(context.Background(), opts)
