...
func (a *api) GetCommitBySha(sha string) (error, *github.RepositoryCommit) {
	...
	_ = newrelic.RecordCustomEvent("GetCommitBySha", newrelic.EventParams{Owner: a.Owner, Repo: a.Repo})
	if err != nil {
		return err, nil
	}
...
func (a *api) GetUserByUsername(username string) (error, *github.User) {
	...
	_ = newrelic.RecordCustomEvent("GetUsers", newrelic.EventParams{Owner: a.Owner, Repo: a.Repo})
	if err != nil {
		return err, nil
	}
	return nil, user
...
func (a *api) GetCommitsBySha(sha string) (error, []*github.RepositoryCommit) {
	revisionCommit, _, err := a.Client.Repositories.GetCommit(a.Ctx, a.Owner, a.Repo, sha)
	_ = newrelic.RecordCustomEvent("GetCommitsBySha", newrelic.EventParams{Owner: a.Owner, Repo: a.Repo})
	if err != nil {
		return err, nil
	}
...
func (a *api) GetIssuesAndPrsByCommits(commits []*github.RepositoryCommit) (error, []codefreshSdk.Annotation, []codefreshSdk.Annotation) {
	...
	_ = newrelic.RecordCustomEvent("ListPullRequests", newrelic.EventParams{Owner: a.Owner, Repo: a.Repo})
	if err != nil {
		return err, nil, nil
	}
	...
	for _, pr := range allPullRequests {
		...
		for _, commit := range commits {
			...
			if *commit.SHA == *mergeCommitSHA {
				...
				_ = newrelic.RecordCustomEvent("GetIssues", newrelic.EventParams{Owner: a.Owner, Repo: a.Repo})
				if err != nil {
					return err, nil, nil
				}
