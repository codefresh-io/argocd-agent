...
func GetRepositories(token string, host string) ([]RepositoryItem, error) {
	client := buildHttpClient()

	req, err := http.NewRequest("GET", host+"/api/v1/repositories", nil)
	req.Header.Add("Authorization", "Bearer "+token)
	resp, err := client.Do(req)
...
func GetClusters(token string, host string) ([]ClusterItem, error) {
	client := buildHttpClient()

	req, err := http.NewRequest("GET", host+"/api/v1/clusters", nil)
	req.Header.Add("Authorization", "Bearer "+token)
	resp, err := client.Do(req)
